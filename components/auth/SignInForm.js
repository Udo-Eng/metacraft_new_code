...
const [inputs,setInputs] = useState({
 ...
 function setInputsHandler(identifier,inputValue){
    setInputs((currentInputs) => {
        return  {...currentInputs,[identifier] : {value : inputValue , invalid : false}}
    } )
 }
...
function clearUserInputs(){
 ...
 function onSubmitHandler(){
    ...
    const emailIsInValid = !(inputs.email.value.trim().length > 0 && inputs.email.value.includes("@"));
    ...
    setInputs((currentInputs) => {
        return  {
            ...
            password: { value : currentInputs.password.value , invalid : passwordIsInValid},
        }}
    );
    ...
    if(emailIsInValid || passwordIsInValid){
        ...
        // Navigate to Dashboard on sucessfully signin
         clearUserInputs();  
  ...
  return (
    <KeyboardAvoidingView>
      ...
      />
      {inputs.password.invalid &&  <InputErrorMessage message="Enter password greater than 6 characters"/>}
